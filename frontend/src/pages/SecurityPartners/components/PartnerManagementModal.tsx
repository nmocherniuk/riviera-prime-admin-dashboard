  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ px: { xs: 2, sm: 3 }, overflowY: "auto" }}>
        <DetailField label="Partner ID" value={partner ? `#${partner.id}` : "—"} emptyAsDash={false} />

        <Typography sx={sectionLabelSx}>Company & contact</Typography>
        <Grid container spacing={1.5}>
          <Grid size={{ xs: 12, md: 6 }}>
            {readOnly ? (
              <DetailField label="Company name" value={formValues.companyName} />
            ) : (
              <TextField fullWidth size="small" label="Company name" value={formValues.companyName} onChange={handleChange("companyName")} sx={modalTextFieldSx} />
            )}
          </Grid>
          <Grid size={{ xs: 12, md: 6 }}>
            {readOnly ? (
              <DetailField label="Contact person" value={formValues.contactPerson} />
            ) : (
              <TextField fullWidth size="small" label="Contact person" value={formValues.contactPerson} onChange={handleChange("contactPerson")} sx={modalTextFieldSx} />
            )}
          </Grid>
          <Grid size={{ xs: 12, md: 6 }}>
            {readOnly ? (
              ...
              <TextField fullWidth size="small" type="email" label="Email" value={formValues.email} onChange={handleChange("email")} sx={modalTextFieldSx} />
            )}
          </Grid>
          <Grid size={{ xs: 12, md: 6 }}>
            {readOnly ? (
              ...
              <TextField fullWidth size="small" label="Phone" value={formValues.phone} onChange={handleChange("phone")} sx={modalTextFieldSx} />
            )}
          </Grid>
          <Grid size={{ xs: 12 }}>
            {readOnly ? (
              <DetailField label="Location / service area" value={formValues.locationServiceArea} />
            ) : (
              <TextField fullWidth size="small" label="Location / service area" value={formValues.locationServiceArea} onChange={handleChange("locationServiceArea")} sx={modalTextFieldSx} />
            )}
          </Grid>
          <Grid size={{ xs: 12, md: 6 }}>
            {readOnly ? (
